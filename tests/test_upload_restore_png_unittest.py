...
class TestUploadRestorePng(unittest.TestCase):
    def test_restore_character_updates_sheet_and_file(self):
        with TemporaryDirectory() as tmpdir:
            ...
            versions_router.pm = pm

            # Restore to v1; restore itself becomes a new version (+1).
            result = asyncio.run(versions_router.restore_version(project_name, "characters", char_name, 1))

            self.assertTrue(result["success"])
            self.assertEqual(result["restored_version"], 1)
            self.assertEqual(result["new_current_version"], 2)
            self.assertEqual(result["file_path"], f"characters/{char_name}.png")

            # project.json should be updated to point to the normalized .png path.
            project2 = pm.load_project(project_name)
            self.assertEqual(project2["characters"][char_name]["character_sheet"], f"characters/{char_name}.png")
            ...
            with Image.open(current_file) as restored:
                ...
                self.assertEqual(restored_rgba.getpixel((0, 0)), (255, 0, 0, 255))
